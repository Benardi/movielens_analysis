...
#   data %>%
#   mutate(century_shuffled = sample(xx_century, n()))
...
tibble(diferenca = diffs) %>% 
  ...
  geom_vline(xintercept = (-1) * diff.prop.obsrvd) +
...
tibble(diferenca = diffs) %>%
  ...
  geom_boxplot() +
  geom_hline(yintercept = diff.prop.obsrvd) +
  geom_hline(yintercept = (-1) *diff.prop.obsrvd) 
```
