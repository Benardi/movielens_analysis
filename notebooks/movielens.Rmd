...
data %>%
  group_by(movieId) %>%
  summarise(median_rating = median(rating), # Deriving whether a movie is well rated
            well_rated = median_rating > 3.5) -> summarised
...
* Heterogeneous movies (movies with more than one genre) are significantly better rated than the Homogeneous movies (movies with only one genre)
